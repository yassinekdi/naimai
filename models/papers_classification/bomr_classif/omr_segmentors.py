...
class OMR_Text_Segmentor:
    ...
    def get_sentence_label(self, sentence, last_token=0):
        ...
        if not self.range_wids_labels:
            self.range_wids_labels = self.ner_processor.get_range_wids_labels()
        sentence_class = self.ner_processor.get_label_with_most_overlap(sentence_range_wids=sentence_range_wids,
                                                                        range_wids_labels=self.range_wids_labels)
    ...
    def segment(self,check_background=False) -> dict:
        '''
        segment naively the text & if check_background, process each background classes : get matched objective sentences with objective classifier.
